...
def compress_audio_worker(arguments):
    file_path, output_directory, target_size_mb = arguments
    compress_audio_file(file_path, output_directory, target_size_mb)
...
def main():
    ...
    with multiprocessing.Pool() as pool:
        # Map each file to the worker function
        pool.map(
            compress_audio_worker,
            [
                (file_path, output_directory, args.target_size_mb)
...
if __name__ == "__main__":
    ...
    main()  # Now, main will configure its logging and start the pool
